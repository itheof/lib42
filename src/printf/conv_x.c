...
static int	formatting_x(t_buffer *pb, t_format *fmt, char *arg, size_t arglen)
{
	...
	if (fmt->conv == 'p')
		...
		buffer_ncat(pb, (fmt->conv == 'X') ? "0X" : "0x", 2);
...
int			conv_x(t_buffer *pb, t_format *fmt, va_list ap)
{
	char	*arg;
...
int			conv_p(t_buffer *pb, t_format *fmt, va_list ap)
{
	char	*arg;
